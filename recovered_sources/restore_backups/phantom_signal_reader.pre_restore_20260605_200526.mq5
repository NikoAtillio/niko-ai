...
double MapPythonQtyToLots(double pythonQty, double signalAccountSize)
{
    ...
    // account_size is embedded in the signal by the Python EA (the --capital value).
    // Scale lots proportionally so the strategy behaves identically on any account size.
    double baseSignalAccountSize = signalAccountSize;
    if(baseSignalAccountSize <= 0.0)
        baseSignalAccountSize = liveAccountSize; // 1:1 if signal has no account_size field
...
datetime _parse_signal_time(string ts)
{
    ...
    // Signals are written in UTC by the Python EA.
    // StringToTime() treats the string as broker server time, so we correct by adding
    // the broker's actual UTC offset, which is computed automatically from the platform.
    // In the Strategy Tester the server time IS used for bar matching, so this is correct.
    datetime parsed = (datetime)StringToTime(ts);
    int brokerOffsetSecs = (int)(TimeCurrent() - TimeGMT());
    return parsed + (datetime)brokerOffsetSecs;
}
...
ulong FindPositionTicketBySignalId(const string &signalId)
{
    // Positions are opened with comment "Phantom|" + signalId.
    // Modify/close signals carry only the bare signalId so we must
    // reconstruct the full comment string before comparing.
    string targetComment = "Phantom|" + signalId;
    ...
    for(int index = 0; index < total; index++)
    {
        ...
        if(positionComment == targetComment)
            return ticket;
    }
    return 0;
}
...
void LoadSignals()
{
    ...
    while(!FileIsEnding(handle))
    {
        ...
        // signal_ts is the bar at which THIS signal should fire (differs from entry_ts
        // for modify/close signals where entry_ts is the original trade open time).
        // Fallback to entry_ts for open signals and legacy files without signal_ts.
        string signalTsStr = _get_json_string(line, "signal_ts");
        if(StringLen(signalTsStr) == 0)
            signalTsStr = signal.entryTs;
        signal.signalTime = _parse_signal_time(signalTsStr);
...
void ProcessLine(const string &line)
{
    ...
    if(action == "modify")
    {
        // modify signals are no longer emitted by Python (removed to fix timing race).
        // This handler is kept for backward compatibility only.
        ulong ticket = FindPositionTicketBySignalId(signalId);
        if(ticket == 0)
        {
            if(InpVerboseLog)
    ...
    if(action == "close")
    {
        ...
        {
            if(InpVerboseLog)
                PrintFormat("Close skipped, no open position for signal_id=%s (already closed by SL/TP)", signalId);
            return;
        }
    ...
    if(InpVerboseLog)
    {
        ...
        if(liveAccountSize <= 0.0)
            liveAccountSize = g_referenceAccountSize;
        double baseSignalAccountSize = accountSize > 0.0 ? accountSize : liveAccountSize;
...
void OnTick()
{
    // Only process signals once per M5 bar
    ...
    // In replay mode Python sends all modify/close signals and fully controls
    // position management — do NOT apply MT5's own trailing on top of that.
    // ManageTrailingStops only runs in live mode as a failsafe if Python
