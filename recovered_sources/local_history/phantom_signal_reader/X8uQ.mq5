...
// Reference account size the Python signals were generated for.
// Set this to match the --capital value used when running the EA script.
// MT5 will scale lots proportionally: live_balance / InpReferenceAccountSize.
// Example: signals from $10,000 Python run on a $70,000 MT5 account → 7x lots.
input double InpReferenceAccountSize = 10000.0;
// Broker server UTC offset in hours (FTMO winter = 2, FTMO summer = 3, UTC broker = 0).
// Signals are written in UTC by the Python EA; this converts them to broker server time
// so replay fires at the correct bar (e.g. UTC 01:00 + offset 2 = EET 03:00).
...
double MapPythonQtyToLots(double pythonQty, double signalAccountSize)
{
    ...
    if(baseSignalAccountSize <= 0.0)
        baseSignalAccountSize = (InpReferenceAccountSize > 0.0) ? InpReferenceAccountSize : 10000.0;
...
datetime _parse_signal_time(string ts)
{
    ...
    // Signal timestamps are in UTC. StringToTime treats the string as broker server time,
    // so we compensate by subtracting the broker offset first (undo that), then add it back
    // as an explicit seconds offset to align to broker bar time.
    // Net effect: parsed = UTC value; then + InpBrokerUTCOffset*3600 = broker server time.
    datetime parsed = (datetime)StringToTime(ts);
    return parsed + (datetime)(InpBrokerUTCOffset * 3600);
}
...
void ProcessLine(const string &line)
{
    ...
    if(InpVerboseLog)
    {
        ...
        if(liveAccountSize <= 0.0)
            liveAccountSize = g_referenceAccountSize;
        double baseSignalAccountSize = accountSize > 0.0 ? accountSize : (InpReferenceAccountSize > 0.0 ? InpReferenceAccountSize : 10000.0);
...
void OnTick()
{
    ...
    // Manage trailing stops once per bar (OnTimer doesn't fire in tester)
    ManageTrailingStops();
}
