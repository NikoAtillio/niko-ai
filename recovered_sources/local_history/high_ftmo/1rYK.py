...
def run_scenario(
    ...
    for bar_i, ts in enumerate(c_idx):
        ...
        if ts_pd >= challenge_end_ts:
            ftmo_hard_stop = True
        if equity_now <= daily_floor or equity_now <= total_floor:
            ftmo_hard_stop = True
        if equity_now >= profit_target_equity and len(traded_days) >= ftmo['min_trading_days']:
            ftmo_hard_stop = True
...
def main():
    ...
    print(f"  Session: {inst_cfg['session_start']:02d}:00–{inst_cfg['session_end']:02d}:00 UTC"
        ...
        print(f"  FTMO: Target {FTMO_CONFIG['profit_target_pct']:.1f}% | "
