...
# FTMO profile: reduce sizing and disable peak-hour boosting.
HIGH_RISK_PCT_MULT = 1.0
HIGH_PEAK_SESSION_BOOST = 1.0
...
def load_csv(path: str) -> pd.DataFrame:
    ...
    if 'date' in df.columns and 'time' in df.columns:
        ...
        # CSV times are in NYSE local time (EST/EDT), convert to UTC for consistency
        df['datetime'] = pd.to_datetime(date_str + ' ' + time_str, errors='coerce')
        # Localize to EST (note: EST is UTC-5, but pytz uses static offsets)
        # For US100: January = EST (UTC-5), Summer = EDT (UTC-4)
        try:
            import pytz
            nyc_tz = pytz.timezone('America/New_York')
            df['datetime'] = df['datetime'].dt.tz_localize(None).dt.tz_localize(nyc_tz, ambiguous='NaT', nonexistent='NaT')
            # Convert to UTC
            df['datetime'] = df['datetime'].dt.tz_convert('UTC')
        except Exception as e:
            # Fallback: assume fixed EST (UTC-5) if pytz not available
            df['datetime'] = df['datetime'] - pd.Timedelta(hours=5)
            print(f"Note: Using fixed EST offset (UTC-5). Conversion issue: {e}")
