...
"""
PHANTOM v4.2 — 5M regime-tuned walk-forward engine
===================================================
...
RUN COMMAND:
    python phantom_v4_2_backtest.py \
...
# Layer 2 — Micro Trap
SWING_LOOKBACK       = 20          # candles to define swing high/low (V4.2)
...
ZONE_MERGE_PCT       = 0.010       # 1.0% zone tolerance (V4.2)
ZONE_MIN_AGE_BARS    = 3           # minimum zone age in bars (V4.2)
...
ATR_PERIOD           = 14
ATR_STOP_MULT        = 0.8
FULL_EXIT_R          = 2.5         # single exit at 2.5R on 5m (V4.2)
...
ATR_REGIME_MULT      = 0.60        # current ATR must be >= 60% of 50-bar avg (V4.2)
...
def run_window(df_1m: pd.DataFrame, df_h1: pd.DataFrame,
    ...
    df_1m['ema50']     = compute_ema(df_1m['close'], EMA5M_PERIOD)
    ...
    for i in range(max(SWING_LOOKBACK + 2, ATR_REGIME_PERIOD + 1), len(df_1m)):
        ...
        # 5m EMA50 trend filter
        if EMA5M_TREND_FILTER:
            ema50 = candle['ema50']
            if pd.isna(ema50):
                skip_reasons['no_ema50'] += 1
                continue
            if direction == 'long' and close < ema50:
                skip_reasons['below_ema50'] += 1
                continue
            if direction == 'short' and close > ema50:
                skip_reasons['above_ema50'] += 1
                continue
...
def run_walkforward(df_full: pd.DataFrame, starting_capital: float,
    ...
    """
    print("\n" + "="*70)
    print("  PHANTOM v4.2 — WALK-FORWARD BACKTEST")
    print("="*70)
