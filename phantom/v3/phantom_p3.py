"""
PHANTOM p3 - Multi-Timeframe Backtest Engine
=============================================
...
Scenarios
    p3C : M5 entry | risk=0.40% | score>=3 | no timeout
    p3B : M5 entry | risk=0.70% | score>=3 | no timeout
    p3A : M1 entry | risk=0.35% | score>=5 | vol filter | no timeout

Usage
    python phantom_p3.py --instrument XAU \\
        --m1 path/M1.csv --m5 path/M5.csv --h1 path/H1.csv --h4 path/H4.csv \\
        --daily path/Daily.csv \\
        [--scenario p3A] [--capital 5000]
...
def canonicalize_scenario(raw: str) -> str:
    ...
    if normalized not in SCENARIOS:
        raise ValueError(f"Unknown scenario '{raw}'. Use p3A | p3B | p3C | ALL")
    return normalized
