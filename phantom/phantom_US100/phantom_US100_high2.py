...
# ZONE DETECTION
# ══════════════════════════════════════════════════════════════════════════════
def add_or_merge_zone(zone_ts, zone_px, zone_dir, confirmed_at, price, direction, merge_tol):
    """Append a zone or merge it into a nearby same-direction zone."""
    for i in range(len(zone_px)):
        if zone_dir[i] != direction:
            continue

        zone_diff = abs(price - zone_px[i]) / zone_px[i]
        if zone_diff > merge_tol:
            continue
        ...
        if confirmed_at > zone_ts[i]:
            zone_ts[i] = confirmed_at
        return

    zone_ts.append(confirmed_at)
    zone_px.append(price)
    zone_dir.append(direction)
...
def build_h4_zones(h4: pd.DataFrame, pivot_bars: int = 2, lookback: int = 50):
    ...
    merge_tol = DEFAULTS['conf_tol'] * 1.5
    ...
    for i in range(pivot_bars, n - pivot_bars):
        ...
        if all(highs[i] >= highs[i - j] for j in range(1, pivot_bars + 1)) and \
           all(highs[i] >= highs[i + j] for j in range(1, pivot_bars + 1)):
            add_or_merge_zone(zone_ts, zone_px, zone_dir, confirmed_at, highs[i], 'short', merge_tol)
        ...
        if all(lows[i] <= lows[i - j] for j in range(1, pivot_bars + 1)) and \
           all(lows[i] <= lows[i + j] for j in range(1, pivot_bars + 1)):
            add_or_merge_zone(zone_ts, zone_px, zone_dir, confirmed_at, lows[i], 'long', merge_tol)
