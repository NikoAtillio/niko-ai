"""
PHANTOM p2 - Multi-Timeframe Backtest Engine (V2 - EST-Aligned H4 Bars)
========================================================================
...
# ── Indicator diagnostic window ───────────────────────────────────────────────
_DIAG_START = pd.Timestamp('2025-12-01')
_DIAG_END   = pd.Timestamp('2025-12-06')
...
def run_scenario(
    ...
    for bar_i, ts in enumerate(c_idx):
        ...
        # ── INDICATOR DIAGNOSTIC: Dec 1-5, 2025 (V6: H4 uses closed bar) ──
        if _DIAG_START <= ts_pd < _DIAG_END:
            session_mult_diag = get_session_size_mult(ts_pd, inst_cfg)
            if session_mult_diag > 0.0:
                _i_h4  = np.searchsorted(h4_idx,  ts, side='left') - 1
                _i_m5  = np.searchsorted(m5_idx,  ts, side='right') - 1
                _i_m15 = np.searchsorted(m15_idx, ts, side='right') - 1
                if _i_h4 >= 0 and _i_m5 >= 0 and _i_m15 >= 0:
                    _zones_in_window = int(np.sum(
                        (zone_ts >= ts - np.timedelta64(zone_lookback_bars * 240, 'm')) &
                        (zone_ts < ts)
                    ))
                    print(
                        f"DIAG|{ts_pd.strftime('%Y-%m-%d %H:%M')}|M5"
                        f"|C={price:.2f}"
                        f"|E20={m5_e20[_i_m5]:.2f}|E50={m5_e50[_i_m5]:.2f}|RSI={m5_rsi[_i_m5]:.2f}"
                        f"|H4|E20={h4_e20[_i_h4]:.2f}|E50={h4_e50[_i_h4]:.2f}|RSI={h4_rsi[_i_h4]:.2f}"
                        f"|ATR={atr_h4_v:.4f}|M15ATR={m15_atr_arr[_i_m15]:.4f}|Zones={_zones_in_window}",
                        flush=True
                    )
        # ─────────────────────────────────────────────────────────────────
