...
ENGINE_VERSION = 'p2_ftmo_v3'   # FTMO-only build (Option B split): fixed start_cap base, 1:30 leverage, static 8% floor
...
FTMO_CONFIG = {
    'account_size': 0.0,            # auto = fixed start_cap captured at run start (F1)
    'max_loss_pct': 8.0,            # STATIC floor: 8% of fixed start_cap (FTMO real 10%)
    'max_daily_loss_pct': 4.5,      # daily floor: 4.5% of fixed start_cap (FTMO real 5%)
    'soft_stop_ratio': 0.8,         # pause before hard floors
    'manual_resume_file': 'tmp/ftmo_resume.flag',
    'hard_close_on_trigger': True,
    'max_leverage': 30.0,           # 1:30 leverage cap (F6 — auto-detect fallback default)
}
# NOTE (build-spec): profit_target_pct and min_trading_days are REMOVED in this
# FTMO-only build. FTMO mode is always-on (no DetectMode / --disable-ftmo).
...
# ════
# NEW TREND-LAYER INDICATORS  (A — shared, identical in FTMO + Cash files)
...
# ════
# NEW TREND-STATE LOGIC  (B — shared, identical in FTMO + Cash files)
...
def run_scenario(
    ...
    skipped    = {'session': 0, 'cluster': 0, 'confirm': 0, 'circuit': 0,
                  ...
                  'not_bounce': 0, 'ftmo': 0, 'trend': 0}
    last_entry = None
    last_loss_exit = None
    debug_rows = [] if debug else None
    debug_tol_mult = 5.0

    # ── FTMO sizing/guardrail base ────
    # F1 FIX: the FTMO risk + guardrail base is the FIXED start_cap and NEVER
    # compounds. account_size is anchored to start_cap for the whole run.
    ftmo = dict(FTMO_CONFIG)
    ftmo['account_size'] = max(float(start_cap), 1.0)
    ...
    for bar_i, ts in enumerate(c_idx):
        ...
        # All FTMO floors are anchored to the FIXED start_cap (F1) — they never
        # compound. total_floor is the STATIC 8% max-loss floor.
        ...
        for z_i in range(zone_start, zone_end):
            ...
            if hard_skip:
                ...
                if debug_rows is not None:
                    debug_rows.append({
                        'ts': ts_pd, 'price': price, 'event': 'zone_skip',
                        'reason': 'trend_hard_skip', 'zone_ts': z_ts,
            ...
            # Stack limit depends on trend alignment (with_trend), confidence, and FTMO DD tier.
            if ftmo_dd_tier >= 1:             # >2% drawdown: max 1 concurrent
                stack_limit = 1
            elif with_trend and conf_mult >= DEFAULTS['confidence_mult']:
            ...
            # ── Position sizing — FIXED start_cap base (F1) + DD tier multiplier ──
            # F1 FIX: notional/risk base is the fixed start_cap and NEVER compounds.
            # (NOTIONAL_CAP is now redundant but retained as a defensive ceiling.)
            notional_base = min(start_cap, NOTIONAL_CAP)
...
def main():
    ...
    # FTMO build: always-on. No --disable-ftmo / --cash-* / profit-target flags.
    parser.add_argument('--ftmo-max-leverage', type=float, default=30.0,
                    help='FTMO leverage cap (F6: 1:30 fallback when broker leverage unknown)')
    ...
    # FTMO guardrails are always-on in this build (no disable path).
    ...
    # F6: leverage cap, 1:30 fallback default.
    FTMO_CONFIG['max_leverage'] = max(1.0, float(args.ftmo_max_leverage))
    reset_signal_file()
    ...
    print(
        f"  FTMO: Max Daily Loss {FTMO_CONFIG['max_daily_loss_pct']:.1f}% | "
