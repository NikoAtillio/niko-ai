//+------------------------------------------------------------------+
//|                                          Phantom_P2_US100_V7.mq5 |
//|                                    Multi-Timeframe Zone Strategy  |
//|                                             US100 Scenario B      |
//+------------------------------------------------------------------+
//  V7 changelog (Production - Diagnostics Removed):
//    - All V6 patches preserved: EWM ATR, H4 closed-bar lookup, UTC auto-detect
//    - Diagnostic instrumentation removed: DIAG_* constants, diagnostic print blocks
//    - Strategy logic, entry/exit conditions, risk parameters unchanged
//    - Ready for live deployment on real account
//+------------------------------------------------------------------+
...
//+------------------------------------------------------------------+
//| V6: CalcEWM_ATR_Py — H4 uses closed bar (shift+1)               |
...
//|   V6: H4 always reads last CLOSED bar (never forming one)        |
//+------------------------------------------------------------------+
                       ...
                       datetime barTime, bool logComparison = false)
{
   ...
   // V6: H4 always uses last CLOSED bar (never the forming one)
   if(tf == PERIOD_H4)
   ...
   // V6: Validation logging — first 10 H4 ATR calls only
   ...
   if(logComparison && tf == PERIOD_H4 && debugCount < 10 && InpEnableDebugPrint)
   {
      ...
      PrintFormat(
         "V6_EWM_ATR[%02d]: shift=%d | EWM=%.4f | Wilder=%.4f | Py_Expected=%.4f",
         debugCount, targetShift, ewmATR, wilderVal, InpPythonExpectedATR);

      debugCount++;
      if(debugCount >= 10)
         Print("V6: EWM ATR validation logging complete (10 prints)");
   }
...
//+------------------------------------------------------------------+
//| Entry conditions                                                  |
//| V6 change: H4 uses closed bar (shift+1), logComparison=true     |
//+------------------------------------------------------------------+
void CheckEntryConditions() {
   ...
   // V6: EWM ATR — logComparison=true for H4 (validation prints), false for M15
