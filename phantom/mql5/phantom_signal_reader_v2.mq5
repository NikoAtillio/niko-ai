...
void GetVolumeSpec(double &volMin, double &volStep, double &volMax, int &volDigits)
{
    ...
    long digits = 0;
    if(SymbolInfoInteger(_Symbol, SYMBOL_VOLUME_DIGITS, digits))
        volDigits = (int)digits;
...
double AdjustVolume(double requested)
{
    ...
    // clamp
    if(requested < vol_min) requested = vol_min;
    ...
    if(adjusted < vol_min) adjusted = vol_min;
...
void ProcessLine(const string &line)
{
    ...
    double useQty = AdjustVolume(qty);
    if(useQty <= 0.0)
    {
    ...
    if(useQty != qty && InpVerboseLog)
        PrintFormat("Adjusted qty %.8f -> %.8f to match symbol step/min", qty, useQty);
...
int OnInit()
{
    ...
    PrintFormat("phantom_signal_reader initialized v1.3-gated | magic=%d | mode=%s | replay=%s | start_line=%d | loaded=%d",
                     InpMagicNumber,
    ...
    int vol_digits;
    GetVolumeSpec(vol_min, vol_step, vol_max, vol_digits);
    PrintFormat("volume constraints | symbol=%s | min=%.8f | step=%.8f | max=%.8f | digits=%d",
                _Symbol,
                vol_min,
                vol_step,
                vol_max,
