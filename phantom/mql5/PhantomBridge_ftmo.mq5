...
//|  FTMO-HARDCODED FORK of PhantomBridge_v2                         |
...
//|    [FTMO-1] Force BROKER_FTMO — removed DetectMode/auto-detect   |
//|    [FTMO-2] Static 8% max-loss floor off fixed start_cap         |
//|    [FTMO-3] Daily loss 4.5% of fixed start_cap                   |
//|    [FTMO-4] Soft stop ratio 0.8 (80% circuit breaker)            |
//|    [FTMO-5] InpUsePythonSizing=true by default                   |
//|    [FTMO-6] Stripped CASH-only inputs/logic (tiered risk,        |
//|             trailing max-loss, lot cap mult, withdrawal reanchor)|
//|    [FTMO-7] Stripped profit_target / min_trading_days (removed)   |
//|    [FTMO-8] Per-login state isolation preserved                   |
//+----+
#property strict
#property description "PHANTOM p2 bridge (FTMO) - reads phantom_signals.jsonl and mirrors Python signals"
...
void   SaveState();
void   LoadState();
// [FTMO-6] TieredRiskPct removed — FTMO uses flat InpFtmoRiskPct
...
// --- broker mode --- [FTMO-1] Hardcoded FTMO, no auto-detect
enum ENUM_BROKER_MODE { BROKER_FTMO=1 };
input ENUM_BROKER_MODE InpBrokerMode = BROKER_FTMO;            // FORCED: FTMO mode only

// --- account baseline ---
input double  InpStartCapOverride    = 10000.0;                 // fixed risk-base anchor (FTMO challenge size)

// --- FTMO guardrails --- [FTMO-2/3/4]
input double  InpDailyLossPct        = 4.5;                     // daily loss limit, % of fixed start_cap (FTMO real 5%)
input double  InpMaxLossPct          = 8.0;                     // max loss limit, % of fixed start_cap (FTMO real 10%)
input double  InpCircuitBreakerPct   = 80.0;                    // soft stop: pause at 80% of daily limit amount
// [FTMO-7] No profit_target or min_trading_days inputs
...
// --- manual resume after a hard pause ---
...
input bool    InpUsePythonSizing     = true;                    // [FTMO-5] TRUE by default — trust signal qty
...
// [FTMO-1] g_mode is always BROKER_FTMO
ENUM_BROKER_MODE g_mode = BROKER_FTMO;
...
// [FTMO-6] g_last_balance removed — no withdrawal re-anchor in FTMO
datetime g_current_day    = 0;

// account baseline
double   g_start_cap      = 0.0;     // fixed original account size
// [FTMO-6] g_peak_equity kept only for state persistence compatibility but not
//           used for trailing floor (FTMO uses static floor off start_cap)
double   g_peak_equity    = 0.0;
long     g_login          = 0;       // [FTMO-8] per-account state isolation
...
bool LoadAllEvents()
{
   ...
   PrintFormat("PhantomBridge FTMO replay loaded %d events", ArraySize(g_replay_raw));
   return true;
}
...
// [FTMO-1] DetectMode() removed — g_mode is always BROKER_FTMO

//==== GUARDRAILS (FTMO static floors) ====
datetime ServerDay()
{
...
void ResetDayIfNeeded()
{
   ...
   if(d!=g_current_day){
      ...
      if(g_halted_today && g_halt_serverday!=d){
         ...
         Notify("PHANTOM FTMO resumed","New server day "+TimeToString(d,TIME_DATE)+". Daily halt cleared; trading resumed.");
         SaveState();
      }
   }
}

// [FTMO-6] TieredRiskPct() removed — FTMO uses flat InpFtmoRiskPct on fixed start_cap
...
// [FTMO-2/3/4] FTMO guardrail — all floors are STATIC off fixed start_cap
bool GuardrailBlock()
{
   if(g_disabled_perm){
   ...
   // [FTMO-2] peak tracking kept for state persistence but not used for floor calc
   if(eq > g_peak_equity){ g_peak_equity = eq; }

   // ---- daily loss amount: FTMO = fixed % of start_cap ---- [FTMO-3]
   double daily_amount = g_start_cap * (InpDailyLossPct/100.0);
   double daily_floor   = g_day_start_balance - daily_amount;
   double daily_loss    = g_day_start_equity - eq;
   double breaker_level = daily_amount * (InpCircuitBreakerPct/100.0);  // [FTMO-4]

   // ---- circuit breaker: soft stop at 80% of daily amount ----
   if(daily_loss >= breaker_level && daily_loss < daily_amount){
      ...
      Notify("PHANTOM FTMO breaker","Hit "+DoubleToString(InpCircuitBreakerPct,0)+"% of daily loss limit (loss="+
   ...
   if(eq <= daily_floor){
      ...
      g_halted_today=true;
      g_halt_serverday=ServerDay();
      Notify("PHANTOM FTMO halted","Daily loss floor breached (eq="+DoubleToString(eq,2)+
             " <= floor="+DoubleToString(daily_floor,2)+"). Flattened & halted; auto-resume next day.");
      SaveState();
      return true;
   }

   // ---- max loss floor: STATIC 8% of start_cap ---- [FTMO-2]
   double max_floor = g_start_cap - g_start_cap*(InpMaxLossPct/100.0);

   if(eq <= max_floor){
      FlattenAll("MAX_LOSS");
      g_disabled_perm=true;
      g_halt_serverday=ServerDay();
      Notify("PHANTOM FTMO DISABLED","FTMO max-loss floor breached (eq="+DoubleToString(eq,2)+
...
//==== LOT SIZING (FTMO: fixed-base risk) ====
// [FTMO-1] Only FTMO logic retained; CASH branches removed
double ComputeLotsForSignal(const string dir,const double entry,const double stop,const double qty,const double sacct)
{
   double eq = AccountInfoDouble(ACCOUNT_EQUITY);
   ...
   // FTMO: flat risk off fixed start_cap
   double risk_pct  = InpFtmoRiskPct;
   double risk_base = g_start_cap;
   double risk_amt  = risk_base * (risk_pct/100.0);

   // Cap risk by remaining distance to static max-loss floor
   double max_floor = g_start_cap - g_start_cap*(InpMaxLossPct/100.0);
...
void HandleOpen(const string js)
{
   ...
   if(InpReplayMode && InpReplayUseSignalPricing){
      ...
      // [FTMO-5] Default path: trust Python sizing scaled by equity ratio
...
int OnInit()
{
   ...
   // [FTMO-1] No DetectMode() — always FTMO
   g_mode=BROKER_FTMO;
   trade.SetExpertMagicNumber(InpMagicNumber);
   ...
   // --- account baseline + per-login state --- [FTMO-8]
   ...
   if(g_disabled_perm && InpManualResume){
      g_disabled_perm=false;
      g_peak_equity=AccountInfoDouble(ACCOUNT_EQUITY);
      Notify("PHANTOM FTMO resumed","Manual resume flag set. Hard-pause cleared; peak re-anchored at "+
   ...
   // [FTMO-2] Init risk summary — all floors static off start_cap
   double init_daily_amount = g_start_cap * (InpDailyLossPct/100.0);
   double init_daily_floor  = g_day_start_balance - init_daily_amount;
   double init_max_floor    = g_start_cap - g_start_cap*(InpMaxLossPct/100.0);

   PrintFormat("PhantomBridge FTMO init | symbol=%s digits=%d step=%.2f stops=%d mode=FTMO replay=%s",
               g_symbol,g_digits,g_volstep,g_stopslevel,
               (InpReplayMode?"true":"false"));
   PrintFormat("PHANTOM_RISK_INIT | mode=FTMO start_cap=%.2f daily_floor=%.2f max_floor=%.2f day_start_balance=%.2f peak_equity=%.2f",
       g_start_cap, init_daily_floor, init_max_floor, g_day_start_balance, g_peak_equity);
   LogCSV("RISK_INIT;mode=FTMO"+
      ";start_cap="+DoubleToString(g_start_cap,2)+
   ...
   LogCSV("INIT;symbol="+g_symbol+";mode=FTMO");
...
void OnTick()
{
   ...
   // [FTMO-1] No CASH day-reset branch; FTMO uses ResetDayIfNeeded inside GuardrailBlock
   if(InpReplayMode){
      ReplayDueEvents(bt);
   }
   else {
...
// [FTMO-6] OnTradeTransaction (withdrawal re-anchor) removed — not needed for FTMO prop accounts
