//+------------------------------------------------------------------+
//|                                          Phantom_P2_US100_V8.mq5 |
//|                                    Multi-Timeframe Zone Strategy  |
//|                                             US100 Scenario B      |
//+------------------------------------------------------------------+
//  V8 changelog (V8a + V8b combined):
//
...
#property description "Scenario B - V8 Hard-Seed ATR + Zone Touch Confirmation"
...
// --- V8b: Zone Touch Confirmation ---
input bool     InpZoneTouchRequired   = true;   // Require prior touch+bounce before entry
input int      InpZoneBounceBars      = 1;       // Min M5 bars price must hold away from zone after bounce
input double   InpZoneBounceATRFrac   = 0.3;    // Bounce distance >= this * M15 ATR to confirm
...
input int      InpCTH4ScoreMin        = 0;       // Counter-trend: allow H4 to be flat/against the trade
input int      InpCTH1ScoreMin        = 1;       // Counter-trend: require H1 confirmation
input int      InpCTLTFScoreMin       = 1;       // Counter-trend: require M5 confirmation
input int      InpCTTotalScoreMin     = 4;       // Counter-trend: higher total confidence threshold
...
void CheckEntryConditions() {
   ...
   for(int i = 0; i < m_zoneCount; i++) {
      ...
      if(counterTrend)
      {
         if(h4Score < InpCTH4ScoreMin || h1Score < InpCTH1ScoreMin || ltfScore < InpCTLTFScoreMin) continue;
