//+------------------------------------------------------------------+
//|                                          Phantom_P2_US100_V4.mq5 |
//|                                    Multi-Timeframe Zone Strategy  |
//|                                             US100 Scenario B      |
//+------------------------------------------------------------------+
//  V4 changelog (vs V2):
//    - Auto-detect UTC offset with manual override fallback
//    - SafeBarShift() from V3 (edge case fix) — applied everywhere
//    - InpSpreadBPS / InpSlippageBPS default → 0.0
//    - Session diagnostic logging in OnInit()
//    - Get4HWindowStart() uses pure arithmetic (DST-safe)
//    - Wilder ATR (iATR) retained from V2 (proven profitable)
//    - All strategy logic verbatim from V2
//+------------------------------------------------------------------+
#property copyright "Phantom P2 MT5"
#property version   "4.00"
#property description "Multi-Timeframe Zone-Based Strategy for US100"
#property description "Scenario B - V4 Clean Build"
...
input bool     InpAutoDetectUTC = false;      // Auto-detect broker UTC offset
input int      InpManualUTCOffset = -5;       // Manual override (only if auto-detect off)
...
// Indicator handles (V4: Wilder ATR retained)
...
string        m_tradeCsvFileName = "phantom_mql5_trade_log_v4.csv";
...
//+------------------------------------------------------------------+
//| V4: SafeBarShift – matches Python searchsorted(side='right')-1   |
//+------------------------------------------------------------------+
...
//+------------------------------------------------------------------+
//| V4: UTC helpers using detected offset                            |
//+------------------------------------------------------------------+
int GetEffectiveUTCOffset()
{
   return m_detectedUTCOffsetHours;
}

datetime ToUTC(datetime serverTime)
{
   return serverTime - (m_detectedUTCOffsetHours * 3600);
}

datetime FromUTC(datetime utcTime)
{
   return utcTime + (m_detectedUTCOffsetHours * 3600);
}

void GetUTCTime(MqlDateTime &utc)
{
   TimeToStruct(ToUTC(TimeCurrent()), utc);
}

//+------------------------------------------------------------------+
//| V4: DST-safe 4H window start — matches Python ts.floor('4h')    |
//+------------------------------------------------------------------+
datetime Get4HWindowStart(datetime timeUtc)
{
   return (timeUtc / 14400) * 14400;  // Integer floor to 4H boundary (14400 seconds)
}

//+------------------------------------------------------------------+
//| CSV logging (unchanged from V2)                                  |
//+------------------------------------------------------------------+
bool OpenTradeCsv() {
...
//+------------------------------------------------------------------+
//| Export functions (unchanged from V2)                             |
//+------------------------------------------------------------------+
void ExportTesterTrades() {
   string fileName = "phantom_mt5_tester_export_v4.csv";
   ...
   for(int i = 0; i < dealsTotal; i++) {
      ...
      double rValue = (initialRisk > 0.0) ? ((direction == 1) ? (exitPrice - entryPrice) / initialRisk : (entryPrice - exitPrice) / initialRisk) : 0.0;
      string exitReason = InferExitReason(exitPrice, stopPrice, tpPrice);
      double holdingMinutes = (double)(exitTime - entryTime) / 60.0;

      FileWrite(handle,
                TimeToString(entryTime, TIME_DATE | TIME_SECONDS), TimeToString(exitTime, TIME_DATE | TIME_SECONDS),
                ...
                DoubleToString(sessionMult, 2), DoubleToString(regimeMult, 2),
                DoubleToString(zonePrice, _Digits), TimeToString(zoneTimeUtc, TIME_DATE | TIME_SECONDS),
...
void ExportAllDealsToCSV() {
   int fileHandle = FileOpen("phantom_mt5_export_v4.csv", FILE_WRITE | FILE_CSV | FILE_ANSI | FILE_COMMON, ';');
   ...
   for(int i = 0; i < dealsTotal; i++) {
      ...
      FileWrite(fileHandle, TimeToString(entryTime, TIME_DATE | TIME_SECONDS), TimeToString(exitTime, TIME_DATE | TIME_SECONDS),
                (long)positionId, Symbol(), (direction == 1) ? "long" : "short",
                DoubleToString(HistoryDealGetDouble(deal, DEAL_VOLUME), 2),
                DoubleToString(entryPrice, _Digits), DoubleToString(exitPrice, _Digits),
                DoubleToString(stopPrice, _Digits), DoubleToString(tpPrice, _Digits),
                DoubleToString(exitProfit, 2), DoubleToString(exitCommission, 2),
                DoubleToString(exitSwap, 2), DoubleToString((exitProfit + exitCommission + exitSwap), 2),
...
int OnInit() {
   ...
   if(InpAutoDetectUTC) {
      ...
      PrintFormat("V4 UTC: Auto-detected offset = %+d hours (Server: %s, GMT: %s)",
                  m_detectedUTCOffsetHours,
                  TimeToString(TimeCurrent(), TIME_DATE|TIME_MINUTES),
                  TimeToString(TimeGMT(), TIME_DATE|TIME_MINUTES));
   } else {
      m_detectedUTCOffsetHours = InpManualUTCOffset;
      PrintFormat("V4 UTC: Manual offset = %+d hours", m_detectedUTCOffsetHours);
   }

   // V4: Session diagnostic
   MqlDateTime utcNow;
   TimeToStruct(ToUTC(TimeCurrent()), utcNow);
   PrintFormat("V4 Session: Current UTC hour=%d | Session window=%d:00-%d:00 | InSession=%s",
   ...
   Print("Phantom P2 US100 Scenario B V4 initialized successfully");
...
//+------------------------------------------------------------------+
//| Zone building (unchanged from V2)                                |
//+------------------------------------------------------------------+
void BuildH4Zones() {
...
//+------------------------------------------------------------------+
//| Position management (unchanged from V2)                          |
//+------------------------------------------------------------------+
void ManageOpenPositions() {
   ...
   for(int i = PositionsTotal() - 1; i >= 0; i--) {
      if(m_position.SelectByIndex(i)) {
         ...
         double currentR = (isLong) ? (barClose - entryPrice) / initialRisk : (entryPrice - barClose) / initialRisk;
         ...
         bool exitNow = false; double exitSignalPx = 0.0;
         if(isLong) {
         ...
         if(exitNow) {
            ...
            double grossProfit = (isLong) ? (exitSignalPx - entryPrice) * volume * SymbolInfoDouble(Symbol(), SYMBOL_TRADE_TICK_VALUE) / SymbolInfoDouble(Symbol(), SYMBOL_TRADE_TICK_SIZE)
                                          : (entryPrice - exitSignalPx) * volume * SymbolInfoDouble(Symbol(), SYMBOL_TRADE_TICK_VALUE) / SymbolInfoDouble(Symbol(), SYMBOL_TRADE_TICK_SIZE);
            ...
            LogTradeCsvRow(m_pos_meta[metaIndex].entry_time_utc, barTimeUtc, ticket,
                          m_pos_meta[metaIndex].direction, volume, entryPrice, exitSignalPx, stopPrice, tpPrice,
                          initialRisk, exitReason, grossProfit, 0, 0, grossProfit, rValue,
                          m_pos_meta[metaIndex].total_score, m_pos_meta[metaIndex].confidence_mult,
...
void LogClosedTradesFromHistory() {
   ...
   for(int i = 0; i < dealsTotal; i++) {
      ...
      for(int j = 0; j < ArraySize(m_pos_meta); j++) { if(m_pos_meta[j].ticket == positionId && m_pos_meta[j].logged) { alreadyLogged = true; break; } }
      ...
      double rValue = (meta.initial_risk > 0.0) ? ((meta.direction == 1) ? (dealPrice - meta.entry_price) / meta.initial_risk : (meta.entry_price - dealPrice) / meta.initial_risk) : 0.0;
      LogTradeCsvRow(meta.entry_time_utc, dealTime, positionId, meta.direction, dealVolume,
                     ...
                     meta.total_score, meta.confidence_mult, meta.regime, DealReasonToString(dealReason),
                     meta.entry_comment, meta.session_mult, meta.regime_mult, meta.zone_price, meta.zone_time_utc, dealComment);
      m_pos_meta[metaIndex].logged = true;
   }
}

//+------------------------------------------------------------------+
//| Entry conditions (unchanged from V2)                             |
//+------------------------------------------------------------------+
void CheckEntryConditions() {
   ...
   // V4: Uses SafeBarShift internally via GetATRAtTime
...
int CountPositions() {
   ...
   for(int i = 0; i < PositionsTotal(); i++) {
      if(m_position.SelectByIndex(i) && m_position.Symbol() == Symbol() && m_position.Magic() == InpMagicNumber) count++;
   }
   return count;
}
...
// V4: SafeBarShift used in GetDailyRegime and GetTFScoreAtTime
...
double GetRegimeMultiplier(string regime, int direction) {
   if((regime == "bull" && direction == 1) || (regime == "bear" && direction == -1)) return 1.0;
...
//+------------------------------------------------------------------+
//| Execute entry (unchanged from V2)                                |
//+------------------------------------------------------------------+
void ExecuteEntry(SZone &zone, double h4ATR, double sessionMult,
   ...
   double takeProfit = (zone.direction == 1) ? entryPrice + (InpTPMult * stopDistance) : entryPrice - (InpTPMult * stopDistance);
   ...
   double riskPerLot = (tickSize > 0.0 && tickValue > 0.0) ? (initialRisk / tickSize) * tickValue : initialRisk * contractSize;
   ...
   string comment = StringFormat("P2_B|%s|S=%d|ATR=%.5f|REG=%s|CONF=%.1f",
                                 (zone.direction == 1) ? "LONG" : "SHORT", totalScore, h4ATR, GetDailyRegime(barTime), confMult);
   ...
   if(m_trade.ResultRetcode() == TRADE_RETCODE_DONE) {
      ulong ticket = 0;
      for(int i = PositionsTotal() - 1; i >= 0; i--) {
         if(m_position.SelectByIndex(i) && m_position.Symbol() == Symbol() && m_position.Magic() == InpMagicNumber) { ticket = m_position.Ticket(); break; }
      }
      if(ticket == 0) return;
...
double ApplyExecutionAdjustment(double price, int direction, bool isEntry) {
   ...
   return (isEntry) ? ((direction == 1) ? price + adj : price - adj) : ((direction == 1) ? price - adj : price + adj);
}

// V4: SafeBarShift applied to ATR lookup
...
int CountEntriesInWindow(datetime entryUtc) {
   ...
   for(int i = 0; i < ArraySize(m_entry_times_utc); i++)
      if(m_entry_times_utc[i] >= ws && m_entry_times_utc[i] < ws + PeriodSeconds(PERIOD_H4)) count++;
   return count;
}
...
//+------------------------------------------------------------------+
//| DrawZones, OnTradeTransaction, FTMO helpers, OnTester           |
//+------------------------------------------------------------------+
void DrawZones() {
   ...
   for(int i = 0; i < m_zoneCount; i++) {
      ...
      ObjectCreate(0, n + "_line", OBJ_TREND, 0, FromUTC(m_zones[i].origin_time_utc), m_zones[i].price, TimeCurrent(), m_zones[i].price);
...
int GetUTCDateKey(datetime utcTime) { MqlDateTime dt; TimeToStruct(utcTime, dt); return dt.year * 1000 + dt.day_of_year; }
