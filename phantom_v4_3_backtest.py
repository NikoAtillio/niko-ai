...
"""
PHANTOM v4.3 — 5M market-sensitive parameter tuning
===================================================
...
ROOT CAUSE FIX:
  V4.2 had stacked 5 filters (SWING_LOOKBACK=20, ZONE_MIN_AGE_BARS=3, EMA5M, ATR_REGIME, 
  MAX_STOP_PCT) creating near-impossible entry. V4.3 loosens each one by 1-2 notches.
  On 5M, a 20-bar swing = 100 minutes to form. Reducing to 10 = 50 minutes (more zones).
  H4 EMA is cleaner than 5M EMA for trend context (less whipsaw-prone).
...
ATR_REGIME_MULT      = 0.4         # current ATR must be >= 40% of 50-bar avg (V4.3: relaxed from 0.6)
...
def parse_args():
    p = argparse.ArgumentParser(description="PHANTOM v4.2 XAUUSD 5M Walk-Forward Backtest")
    ...
    p.add_argument("--outdir",      type=str,   default="phantom_v4_2_output")
...
def run_walkforward(df_full: pd.DataFrame, starting_capital: float,
    ...
    """
    print("\n" + "="*70)
    print("  PHANTOM v4.3 — WALK-FORWARD BACKTEST")
    print("="*70)
    print(f"  Train: {TRAIN_DAYS}d  |  Test: {TEST_DAYS}d  |  Symbol: {symbol}  |  Interval: {INTERVAL}")
