...
"""
PHANTOM US100 v4.3 — 5M regime-tuned for Nasdaq 100
===================================================

CHANGES FROM V4.2:
  SWING_LOOKBACK 20→10        — Reduce zone formation time from 100m to 50m
  ZONE_MIN_AGE_BARS 3→1       — Remove harsh recency gate, accept zones age 1+ bars
  ZONE_MERGE_PCT 0.010→0.008  — Tighter merge tolerance for distinct zones
  EMA5M_TREND_FILTER Disabled — 5M EMA too noisy, remove it entirely
  H4_EMA_TREND_FILTER Added   — Use H4 EMA50 for cleaner high-level trend context
  ATR_REGIME_MULT 0.6→0.4     — Lower bar for volatility regime (less restrictive)
  MAX_STOP_PCT 0.0035→0.005   — Widen stop allowance for 5M candles (more room)
  FULL_EXIT_R 2.5→2.0         — Easier targets, more completions
  MAX_CONCURRENT 2→3          — Allow more simultaneous trades
  Target: 8–15 trades/window (168–315 total across 21 windows)

US100 ADAPTATIONS FROM XAUUSD V4.3:
  FEE_PCT_PER_SIDE: 7e-05 → 5e-05 (lower CFD spread on US100 vs spot gold)
  ATR_REGIME_MULT: 0.4 → 0.35 (US100 more volatile intraday, relax volatility gate)
  All % params (ZONE_MERGE_PCT, MAX_STOP_PCT) unchanged — automatically scale with price
  Key risk: US100 has defined cash hours (14:30–21:00 UTC). Off-hours S/R zones are weak.
  Monitor trade frequency and consider session filter if volume <<= current baseline.

UNCHANGED:
    - All 5 entry layers (H1 EMA trend, micro-trap, volume, momentum, optional session)
...
RUN COMMAND:
    python phantom_v4_3_backtest.py \
...
BASE_RISK_PCT        = 0.01        # 1.0% risk per trade (V4.2)

# Core interval
INTERVAL             = '5m'        # resample M1 data to 5m for signal generation

# Layer 2 — Micro Trap
SWING_LOOKBACK       = 10          # candles to define swing high/low (V4.3: reduced from 20)
...
ZONE_MERGE_PCT       = 0.008       # 0.8% zone tolerance (V4.3: tightened from 0.010)
ZONE_MIN_AGE_BARS    = 1           # minimum zone age in bars (V4.3: relaxed from 3)

# Layer 3 — Volume
VOLUME_ZSCORE_MIN    = 0.0         # relaxed for 5m (V4.2)

# Layer 5 — Session (UTC)
SESSION_FILTER       = False       # disabled in V4.2
SESSION_START_UTC    = 7
SESSION_END_UTC      = 16
...
ATR_PERIOD           = 14
ATR_STOP_MULT        = 0.8
FULL_EXIT_R          = 2.0         # single exit at 2.0R on 5m (V4.3: reduced from 2.5)
...
MIN_RR_ENFORCE       = True
MAX_STOP_PCT         = 0.005       # skip if stop > 0.50% of entry price (V4.3: relaxed from 0.0035)
                                   # on gold ~$3000 this = $15 max stop

# FIX 3: ATR regime filter
ATR_REGIME_FILTER    = True
ATR_REGIME_PERIOD    = 50          # rolling average of ATR over 50 bars
ATR_REGIME_MULT      = 0.35        # current ATR must be >= 35% of 50-bar avg (US100: even more relaxed)
                                   # US100 is more volatile intraday, lower threshold catches more trades

# 5m trend context (disabled in V4.3)
EMA5M_TREND_FILTER   = False       # V4.3: disabled, replaced with H4 EMA
EMA5M_PERIOD         = 50

# H4 EMA trend context (NEW in V4.3)
H4_EMA_TREND_FILTER  = True        # V4.3: high-level trend gate using H4 EMA50
H4_EMA_PERIOD        = 50

# Position limits
MAX_CONCURRENT       = 3            # V4.3: increased from 2
DAILY_LOSS_LIMIT_PCT = 0.02        # -2% daily circuit breaker

# Fees (US100 CFD: lower spread than spot gold)
FEE_PCT_PER_SIDE     = 0.00005    # 0.005% per side (US100 CFD)
...
def parse_args():
    p = argparse.ArgumentParser(description="PHANTOM US100 v4.3 5M Walk-Forward Backtest")
    p.add_argument("--data",        type=str,   required=True,
                   help="Path to MT5 US100 M1 CSV file")
    p.add_argument("--capital",     type=float, default=STARTING_CAPITAL)
    p.add_argument("--risk",        type=float, default=BASE_RISK_PCT,
                   help="Base risk per trade (0.01 = 1.0%%)")
    p.add_argument("--outdir",      type=str,   default="phantom_us100_output")
    ...
    p.add_argument("--symbol",      type=str,   default="US100")
    return p.parse_args()
...
def detect_trap(df: pd.DataFrame, i: int) -> dict | None:
    if i < SWING_LOOKBACK + ZONE_MIN_AGE_BARS:
        return None

    candle   = df.iloc[i]
    lookback = df.iloc[i - SWING_LOOKBACK: i - ZONE_MIN_AGE_BARS]
    if len(lookback) == 0:
        return None
    ...
    touch_pct_bull = abs(candle['low'] - swing_low) / max(abs(swing_low), 1e-9)
    if (SWEEP_TICK_MIN <= sweep_below <= SWEEP_TICK_MAX
            and touch_pct_bull <= ZONE_MERGE_PCT
    ...
    touch_pct_bear = abs(candle['high'] - swing_high) / max(abs(swing_high), 1e-9)
    if (SWEEP_TICK_MIN <= sweep_above <= SWEEP_TICK_MAX
            and touch_pct_bear <= ZONE_MERGE_PCT
...
def run_window(df_1m: pd.DataFrame, df_h1: pd.DataFrame,
    ...
    df_1m['ema50']     = compute_ema(df_1m['close'], EMA5M_PERIOD)  # kept for legacy, not used
    ...
    # H4 EMA computation (NEW in V4.3)
    # Resample 1m data to 4h, calculate EMA50, forward-fill back to 1m index
    if H4_EMA_TREND_FILTER:
        df_4h = df_1m.resample('4h').agg({
            'open': 'first', 'high': 'max',
            'low': 'min', 'close': 'last', 'volume': 'sum'
        }).dropna()
        df_4h['ema50'] = compute_ema(df_4h['close'], H4_EMA_PERIOD)
        df_1m['h4_ema50'] = df_4h['ema50'].reindex(df_1m.index, method='ffill')
    else:
        df_1m['h4_ema50'] = None
    ...
    for i in range(max(SWING_LOOKBACK + 2, ATR_REGIME_PERIOD + 1), len(df_1m)):
        ...
        # Layer 5: Session gate (disabled in V4.2 by default)
        if SESSION_FILTER:
        ...
        # H4 EMA50 trend filter (NEW in V4.3 — cleaner than 5M EMA)
        if H4_EMA_TREND_FILTER:
            h4_ema = candle['h4_ema50']
            if pd.isna(h4_ema):
                skip_reasons['no_h4_ema'] += 1
                continue
            if direction == 'long' and close < h4_ema:
                skip_reasons['below_h4_ema'] += 1
                continue
            if direction == 'short' and close > h4_ema:
                skip_reasons['above_h4_ema'] += 1
                continue
...
def run_walkforward(df_full: pd.DataFrame, starting_capital: float,
    ...
    """
    print("\n" + "="*70)
    print("  PHANTOM US100 v4.3 — WALK-FORWARD BACKTEST")
    print("="*70)
    print(f"  Train: {TRAIN_DAYS}d  |  Test: {TEST_DAYS}d  |  Symbol: {symbol}  |  Interval: {INTERVAL}  |  Range: {df_full.index[0].date()} → {df_full.index[-1].date()}")
    print(f"  Capital: ${starting_capital:,.0f}  |  Base risk: {base_risk_pct*100:.2f}%  |  Fee: {FEE_PCT_PER_SIDE*1e6:.0f}pips")
    print(f"  Filters: swing={SWING_LOOKBACK}b | zone_age={ZONE_MIN_AGE_BARS}b | atr_mult={ATR_REGIME_MULT}"
          f" | h4_ema={H4_EMA_TREND_FILTER} | stop={MAX_STOP_PCT*100:.2f}%")
...
def build_report(all_trades, window_stats, equity_curve,
    ...
    print("\n" + "="*70)
    print("  PHANTOM v4.2 — OVERALL RESULTS")
    print("="*70)
    print(f"\n  Symbol          : {symbol}")
    ...
    csv_path = os.path.join(outdir, "phantom_v4_2_trades.csv")
    trade_df.to_csv(csv_path, index=False)
    print(f"\n  Trades saved    : {csv_path}")

    # Save windows CSV
    if len(ws_df) > 0:
        wcsv = os.path.join(outdir, "phantom_v4_2_windows.csv")
        ws_df.to_csv(wcsv, index=False)
        print(f"  Windows saved   : {wcsv}")

    # Save markdown report
    report_lines = [
        f"# PHANTOM v4.2 Walk-Forward Report\n",
        ...
        f"- **V4.2**: 5M EMA50 trend filter enabled",
        f"- **V4.2**: Zone tolerance={ZONE_MERGE_PCT*100:.2f}% | zone age>={ZONE_MIN_AGE_BARS} bars",
        f"- **V4.2**: Session filter disabled | target={FULL_EXIT_R}R\n",
        f"## Overall Results\n",
        f"- total_trades: {n}",
        f"- win_rate: {wr:.1f}%",
    ...
    report_lines += [
        ...
        f"- INTERVAL: {INTERVAL}",
        f"- ZONE_MERGE_PCT: {ZONE_MERGE_PCT}",
        f"- ZONE_MIN_AGE_BARS: {ZONE_MIN_AGE_BARS}",
        ...
        f"- EMA5M_TREND_FILTER: {EMA5M_TREND_FILTER}",
        f"- EMA5M_PERIOD: {EMA5M_PERIOD}",
    ...
    report_path = os.path.join(outdir, "phantom_v4_2_report.md")
...
def _plot_results(all_trades, equity_curve, window_stats,
    ...
    fig.suptitle(f"PHANTOM v4.2 — {symbol} Walk-Forward Results ({INTERVAL})\n"
    ...
    plt.tight_layout()
    plot_path = os.path.join(outdir, "phantom_v4_2_results.png")
    ...
    if len(ws_df) >= 3:
        try:
            ...
            ax.set_title('PHANTOM v4.2 — Window Metrics Heatmap')
            plt.tight_layout()
            hm_path = os.path.join(outdir, "phantom_v4_2_heatmap.png")
...
def main():
    ...
    # Load data
    df_full_1m = load_mt5_csv(args.data)
    df_full = resample_to_interval(df_full_1m, INTERVAL)
    print(f"  Resampled to {INTERVAL}: {len(df_full):,} candles")
